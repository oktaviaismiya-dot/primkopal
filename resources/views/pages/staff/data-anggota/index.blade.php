...
@section('content')
    ...
    <div class="modal" id="addModal">
        <div class="modal-content">
            ...
            <form action="{{ route('data-anggota.store') }}" method="POST">
                ...
                <label>Pangkat</label>
                <input list="daftarPangkat" name="pangkat" id="pangkatInput" required
                    style="width: 100%; padding: 0.5rem; border-radius: 0.25rem; border: 1px solid #ccc;">

                <datalist id="daftarPangkat">
                    @foreach ($pangkats as $pangkat)
                        <option value="{{ $pangkat->nama }}">
                    @endforeach
                </datalist>

                <div id="pinjamanField" style="display:none; margin-top:1rem;">
                    <label>Maksimal Pinjaman</label>
                    <input type="number" name="maksimal_pinjaman" min="0"
                        style="width: 100%; padding: 0.5rem; border-radius: 0.25rem; border: 1px solid #ccc;">
                </div>
    ...
    <script>
        const input = document.getElementById('pangkatInput');
        const datalist = document.getElementById('daftarPangkat');
        const pinjamanField = document.getElementById('pinjamanField');
        ...
        input.addEventListener('input', function() {
            ...
            // kalau pangkat baru → tampilkan input maksimal pinjaman
            pinjamanField.style.display = found ? 'none' : 'block';
        });
